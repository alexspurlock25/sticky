...
class Table {
    ...
    // static method to delete a row from database table and html table
    deleteRow(row) {
        // grab the value of clicked row
        let btn_value = row.value;
        let body = {
            infoid: btn_value
        }
        // ajax post request to delete row from database
        Ajax.post("http://localhost:3000/delete-row", body).then((response) => {
...
//<button value='"+ row.infoid +"' id='edit-btn-row' class='edit-btn-row' onclick='editRow(this)'><img alt='Edit' src='./images/edit_icon.png'/></button>
        let htmlTableRow = "<tr id='row-" + row.infoid + "' class='"+row.infoid+"'>"
            ...
            + "<td style='display: none;' id='td-pass-stren-graded'>" + row.pass_strength_interpretation + "</td>"
            + "<td style='display: none;' ><button value='"+ row.infoid +"' id='edit-btn-row' class='edit-btn-row' onclick='editRow(this)'><img alt='Edit' src='./images/edit_icon.png'/></button><button value='"+ row.infoid +"' id='delete-btn-row' onclick='this.deleteRow(this)'><img alt='Delete' src='./images/delete_icon.png'/></button></td>"
            + "</tr>";
